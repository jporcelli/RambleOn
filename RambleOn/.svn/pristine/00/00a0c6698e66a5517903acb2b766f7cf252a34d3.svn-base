    ...
    public void actionPerformed(ActionEvent e) {
        ...
        switch (associatedToolTip) {
            ...
            case CREATE_ACCOUNT_OK_TOOLTIP: {
                ...
                //CHECK THE PROPOSED USER NAME AGAISNT THE NAMES OF ALL ACTIVE ACCOUNTS AND FOR THE EMPTY STRING
                if (!(screenManager.getAccountsManager().isNewUserNameUnique(newUserName)) || newUserName.equals("")) {
                    //GIVE FEEDBACK AS TO ALREADY IN USE STATUS OF PROPOSED USER NAME
                    JOptionPane.showMessageDialog(screenManager.getWindow(), "User Name Is Invalid, Please Try Again", "RambleOn", JOptionPane.ERROR_MESSAGE);
                } else {
                    ...
                    //SEE IF ITS THE FIRST TIME AT THE CURRENT ACCOUNT SCREEN
                    if (screenManager.getCurrentAccountScreen() == null) {
                        //IF SO INIT THE SCREEN BEFORE WE SWAP THE PANELS
                        screenManager.initCurrentAccountScreen();
                    }
                    //CLEAR THE TABLE FROM LAST TIME
                    screenManager.resetStatTableCells();
                    ...
                    //REFRESH THE REGIONS PLAYED TREE
                    screenManager.initRegionsPlayedTree();
